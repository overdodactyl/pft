...
pulmonary function tests: predicted values and limits of normal,
z-scores, percent predicted, ATS/ERS pattern classification,
severity grading, bronchodilator response, PRISm screening, and
conditional change scores, all from a data-frame-pipeline API. The
interpretive primitives implement the current ATS/ERS technical
standard (Stanojevic et al. 2022); the predecessor Pellegrino 2005
primitives are also available for reclassification analyses.
...
equation set recommended by the current ATS/ERS technical standard.
To use the predecessor GLI 2012 multi-ethnic equations, pass
`year = 2012` and include a `race` column.
...
in FEV1 or FVC (replacing the 2005 rule of strictly >12% AND >200 mL
from baseline):
...
full ATS/ERS interpretation in one call:
