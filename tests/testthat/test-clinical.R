...
test_that("pft_report renders the spirometry table under year = 2022", {
  # pft_spirometry(year = 2022) writes columns with a `_2022` suffix
  # (e.g. fev1_pred_2022). The report template historically only looked
  # for unsuffixed columns, so the spirometry block came up blank under
  # the GLI Global 2022 equations. Verify the suffix-aware lookup.
  skip_if_not_installed("rmarkdown")
  skip_if_not_installed("knitr")
  d <- data.frame(sex = "M", age = 45, height = 178, race = "Caucasian",
                  fev1_measured = 2.5, fvc_measured = 3.8)
  result <- pft_interpret(d, year = 2022)
  expect_true("fev1_pred_2022" %in% colnames(result))   # precondition

  out <- pft_report(result)
  content <- paste(readLines(out, warn = FALSE), collapse = "\n")

  # The rendered HTML should contain a FEV1 row with the predicted value
  # populated, i.e. a numeric <td> immediately following `>FEV1</td>`,
  # not the placeholder dash that the pre-fix code produced when it
  # couldn't find `fev1_pred`.
  fev1_row <- regmatches(content,
                          regexpr(">FEV1</td>\\s*<td[^>]*>[^<]+</td>", content))
  expect_true(length(fev1_row) == 1)
  expect_false(grepl(">FEV1</td>\\s*<td[^>]*>\\s*-\\s*</td>", fev1_row))
  # Predicted FEV1 for a 45yo 178cm Caucasian male is ~4 L; the
  # formatted cell should at least begin with a digit.
  expect_match(fev1_row, ">FEV1</td>\\s*<td[^>]*>[0-9]")
})
