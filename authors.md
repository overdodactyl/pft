# Authors and Citation

## Authors

- **Pat Johnson**. Author, maintainer.
  [](https://orcid.org/0000-0001-8365-1375)

- **Scott Helgeson**. Author. [](https://orcid.org/0000-0001-7590-2293)

- **Zach Quicksall**. Contributor.

- **Jairo Pena**. Contributor.

## Citation

Source:
[`inst/CITATION`](https://github.com/overdodactyl/pft/blob/main/inst/CITATION)

Johnson P, Helgeson S (2026). *pft: Pulmonary Function Test
Interpretation per ERS/ATS 2022*. R package version 0.1.0.9000,
<https://github.com/overdodactyl/pft>.

    @Manual{,
      title = {pft: Pulmonary Function Test Interpretation per ERS/ATS 2022},
      author = {Pat Johnson and Scott Helgeson},
      year = {2026},
      note = {R package version 0.1.0.9000},
      url = {https://github.com/overdodactyl/pft},
    }

Quanjer P, Stanojevic S, Cole T, et al. (2012). “Multi-ethnic reference
values for spirometry for the 3-95-yr age range: the global lung
function 2012 equations.” *European Respiratory Journal*, **40**(6),
1324-1343.
[doi:10.1183/09031936.00080312](https://doi.org/10.1183/09031936.00080312).

    @Article{Quanjer2012,
      title = {Multi-ethnic reference values for spirometry for the 3-95-yr age range: the global lung function 2012 equations},
      author = {Philip H Quanjer and Sanja Stanojevic and Tim J Cole and {et al.}},
      journal = {European Respiratory Journal},
      year = {2012},
      volume = {40},
      number = {6},
      pages = {1324-1343},
      doi = {10.1183/09031936.00080312},
    }

Bowerman C, Bhakta N, Brazzale D, et al. (2023). “A race-neutral
approach to the interpretation of lung function measurements.” *American
Journal of Respiratory and Critical Care Medicine*, **207**(6), 768-774.
[doi:10.1164/rccm.202205-0963OC](https://doi.org/10.1164/rccm.202205-0963OC).

    @Article{Bowerman2023,
      title = {A race-neutral approach to the interpretation of lung function measurements},
      author = {Cole Bowerman and Nirav R Bhakta and Danielle Brazzale and {et al.}},
      journal = {American Journal of Respiratory and Critical Care Medicine},
      year = {2023},
      volume = {207},
      number = {6},
      pages = {768-774},
      doi = {10.1164/rccm.202205-0963OC},
    }

Hall G, Filipow N, Ruppel G, et al. (2021). “Official ERS technical
standard: Global Lung Function Initiative reference values for static
lung volumes in individuals of European ancestry.” *European Respiratory
Journal*, **57**(3), 2000289.
[doi:10.1183/13993003.00289-2020](https://doi.org/10.1183/13993003.00289-2020).

    @Article{Hall2021,
      title = {Official ERS technical standard: Global Lung Function Initiative reference values for static lung volumes in individuals of European ancestry},
      author = {Graham L Hall and Nicole Filipow and Gregg Ruppel and {et al.}},
      journal = {European Respiratory Journal},
      year = {2021},
      volume = {57},
      number = {3},
      pages = {2000289},
      doi = {10.1183/13993003.00289-2020},
    }

Stanojevic S, Graham B, Cooper B, et al. (2017). “Official ERS technical
standards: Global Lung Function Initiative reference values for the
carbon monoxide transfer factor for Caucasians.” *European Respiratory
Journal*, **50**(3), 1700010.
[doi:10.1183/13993003.00010-2017](https://doi.org/10.1183/13993003.00010-2017).

    @Article{Stanojevic2017,
      title = {Official ERS technical standards: Global Lung Function Initiative reference values for the carbon monoxide transfer factor for Caucasians},
      author = {Sanja Stanojevic and Brian L Graham and Brendan G Cooper and {et al.}},
      journal = {European Respiratory Journal},
      year = {2017},
      volume = {50},
      number = {3},
      pages = {1700010},
      doi = {10.1183/13993003.00010-2017},
    }

Stanojevic S, Kaminsky D, Miller M, et al. (2022). “ERS/ATS technical
standard on interpretive strategies for routine lung function tests.”
*European Respiratory Journal*, **60**(1), 2101499.
[doi:10.1183/13993003.01499-2021](https://doi.org/10.1183/13993003.01499-2021).

    @Article{Stanojevic2022,
      title = {ERS/ATS technical standard on interpretive strategies for routine lung function tests},
      author = {Sanja Stanojevic and David A Kaminsky and Martin R Miller and {et al.}},
      journal = {European Respiratory Journal},
      year = {2022},
      volume = {60},
      number = {1},
      pages = {2101499},
      doi = {10.1183/13993003.01499-2021},
    }
